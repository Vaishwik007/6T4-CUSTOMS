...
/** Brand logo — local SVG fetched by the image pipeline.
 *  Falls back to wordmark in BrandCarousel when file is absent. */
const logoOf = (slug: string) => `/images/brands/${slug}.svg`;

/** Brand hero — real flagship-bike photo of that brand, downloaded by
 *  scripts/fetch-bike-images.ts. Falls back to wordmark if absent. */
