...
/**
 * Each model points to its OWN real photo, downloaded from Wikipedia/Wikimedia
 * Commons by scripts/fetch-bike-images.ts (run `npm run fetch-bikes`).
 * If a file is missing (niche model with no free photo), BikePreview /
 * ModelCarousel fall back to the neutral SVG silhouette — never a wrong bike.
 */
const m = (
  brand: string,
  slug: string,
