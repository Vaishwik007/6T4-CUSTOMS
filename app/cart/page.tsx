...
export default function CartPage() {
  ...
  const subtotal = resolved.reduce((s, x) => s + x.part.price * x.qty, 0);
      ...
      {resolved.length === 0 ? (
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_380px]">
          <ul className="space-y-3">
            {resolved.map((it) => {
              ...
              return (
                <li
                  key={it.partId}
                  className="neon-edge relative grid grid-cols-[1fr_auto] items-center gap-4 border border-white/5 bg-carbon p-5"
                >
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.3em] text-neon">
                      {it.part.brand} · {it.part.category}
                    </p>
                    <h3 className="mt-1 text-display text-lg font-bold uppercase">{it.part.name}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      <div className="text-stencil text-xl text-neon">
                        {formatPrice(it.part.price * it.qty)}
                      </div>
                      <button
                        type="button"
          ...
          <aside className="neon-edge sticky top-24 h-max border border-white/5 bg-carbon p-6">
            ...
            <h2 className="mt-2 text-display text-2xl font-bold uppercase">
              The Damage
            </h2>
            ...
            <Link
              href="/checkout"
              data-cursor="cta"
              className="mt-6 flex items-center justify-center gap-2 bg-neon px-6 py-4 text-display text-xs uppercase tracking-[0.2em] font-bold text-black transition-all hover:bg-white hover:shadow-neon-lg"
            >
